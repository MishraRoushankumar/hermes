...
const SaveRequestToCollectionModal = ({
  ...
  const effectiveCollectionId =
    selectedCollectionId || collectionId || collections?.[0]?.id || "";

  const { mutateAsync, isPending } = useAddRequestToCollection(
    effectiveCollectionId,
  );

  const filteredCollections =
    collections?.filter((collection) =>
      collection.name.toLocaleLowerCase().includes(searchTerm.toLowerCase()),
    ) || [];
      ...
      <div className="space-y-4 ">
        <div>
          ...
          <div className="relative">
            <Input
              className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-100 placeholder-zinc-700"
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
              <span
                className={`text-xs font-bold px-2 py-1 rounded ${requestColorMap[requestData.method]}`}
              >
                {requestData.method}
              </span>
            </div>
        ...
        <div>
          ...
          <div className="relative mb-3">
            ...
            <Input
              type="text"
              placeholder="Search"
              className="w-full bg-zinc-800 border-zinc-700 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              value={searchTerm}
